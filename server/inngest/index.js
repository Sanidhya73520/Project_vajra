...
export const inngest = new Inngest({ id: "project-vajra" });
...
//Inngest Dunction to update workspace data in database
  ...
  async ({ event, step }) => {
    ...
    await step.run(`send-assignment-email`, async () => {
      await sendEmail({
        ...
        <p style="margin:6px 0;"><strong>Description:</strong> ${task.description || "N/A"}</p>
        <p style="margin:6px 0;"><strong>Due Date:</strong> ${task.due_date ? new Date(task.due_date).toLocaleDateString() : "N/A"}</p>
        </div>
    ...
    if (
      ...
      const currentTask = await step.run(`check-if-task-is-completed`, async () => {
        return await prisma.task.findUnique({
          where: { id: taskId },
          include: { assignee: true, project: true },
        });
      ...
      if (currentTask && currentTask.status !== "DONE") {
        await step.run(`send-task-reminder-mail`, async () => {
          await sendEmail({
            ...
            <div style="border: 1px solid #ddd; padding: 12px 16px; border-radius:6px; margin-bottom: 30px;">
            <p style="margin:6px 0;"><strong>Description:</strong> ${currentTask.description || "N/A"}</p>
